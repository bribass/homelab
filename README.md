...
* Debugging of the playbooks can (mostly) be handled using Ansible's `--check` and `--verbose` options:
    ...
    ```
* Passwords are stored as Vault-encrypted entries in the variables file located at `ansible/inventory/group_vars/all.yaml`.
  To add a new password as an encrypted variable:
    1. Run:
        ```
        $ ansible-vault encrypt_string --stdin-name xxx >> ansible/inventory/group_vars/all.yaml
        New Vault password: *****
        Confirm Vault password: *****
        [[ no prompt; enter password to store, followed by Ctrl-D twice ]]
        ````
    2. Edit the variables file `ansible/inventory/group_vars/all.yaml` to rename the variable to the correct 
    ...
    ```
    $ yq -r '.passwd.auth.mgr' ansible/inventory/group_vars/all.yaml | ansible-vault decrypt
    Vault password: *****
